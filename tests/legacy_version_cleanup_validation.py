...
check("cleanup explicitly removes superseded component resources", all(name in cleanup for name in (
    "frontend/AstroFocusStudio.ico", "backend/AstroFocusStudio.ico", "focuser_setup/AstroFocusStudio.ico",
    "frontend/app.res", "backend/app.res", "focuser_setup/app.res",
)))
check("cleanup reports duplicate resources in check-only mode", "duplicate centralized resource files are still present" in cleanup)
...
with tempfile.TemporaryDirectory() as temporary:
    ...
    for duplicate in ("frontend/AstroFocusStudio.ico", "backend/AstroFocusStudio.ico", "focuser_setup/AstroFocusStudio.ico", "frontend/app.res", "backend/app.res", "focuser_setup/app.res"):
        target = temp_root / duplicate; target.parent.mkdir(parents=True, exist_ok=True); target.write_bytes(b"stale")
    ...
    check("functional cleanup removes duplicate centralized resources", all(not (temp_root / duplicate).exists() for duplicate in (
        "frontend/AstroFocusStudio.ico", "backend/AstroFocusStudio.ico", "focuser_setup/AstroFocusStudio.ico",
        "frontend/app.res", "backend/app.res", "focuser_setup/app.res",
    )))
