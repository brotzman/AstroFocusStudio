...
static bool RunBacklashCalibrationCase(int configuredBacklash,int* estimated,int* increment,int* limit,int* guardOut){
    ...
    wchar_t committedText[32];UINT committedValue=0;GetWindowTextW(g_afBacklashEdit,committedText,32);BOOL committed=ParseStrictUIntW(committedText,&committedValue)&&committedValue==(UINT)g_backlashCalEstimated&&g_afBacklash==g_backlashCalEstimated&&g_profileBacklash==g_backlashCalEstimated;
    return !g_backlashCalActive&&guard<6000&&g_backlashCalEstimated>=0&&committed;
}
int main(){
    ...
    total++;passed+=test(ok0&&cal0>=0&&cal0<=inc0&&ok250&&((cal250>=250)?(cal250-250):(250-cal250))<=inc250&&ok1000&&((cal1000>=1000)?(cal1000-1000):(1000-cal1000))<=inc1000&&ok5000&&((cal5000>=5000)?(cal5000-5000):(5000-cal5000))<=inc5000&&lim5000>5000,"3.9.0 simulator: backlash calibration covers 0..5000 and commits autofocus setting");
