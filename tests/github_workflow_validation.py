...
check("metadata:" in workflow and "tr -d '\\r\\n' < VERSION" in workflow, "workflow derives release metadata from VERSION")
check("needs: [metadata, native-tests]" in workflow, "Windows build consumes validated version metadata")
...
check("KNOWN_LIMITATIONS.txt" in workflow, "checkout preflight verifies the current known-limitations payload")
...
for source in (
    ...
    "tests/continuous_focus_logic_validation.cpp",
):
...
check(expected_payload == found_payload, "WiX package contains exactly the current repository payload")
...
check("needs: [metadata, windows-build, bundle-test, msi-test]" in workflow, "tagged release waits for both installer test jobs")
...
check("release_tag:" in workflow and "default: ''" in workflow, "manual release dispatch can inherit the version tag from VERSION")
check("inputs.publish_release == true" in workflow, "release job is gated by the manual publish switch")
check('expected_tag="${{ needs.metadata.outputs.tag }}"' in workflow, "release tag must match the metadata derived from VERSION")
