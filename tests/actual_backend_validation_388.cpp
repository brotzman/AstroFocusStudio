...
int main(){
    ...
    // Version 3.8.8: explicit auto-refocus reference lifecycle and detailed failure classification.
    ...
    // Version 3.8.8 hardening: one shared star limit is authoritative for the reference and field-median autofocus.
    ...
    total++;passed+=test(g_focusNeedScore>=85&&WContains(g_focusNeedLabel,L"Refokus erforderlich")&&WContains(g_focusNeedReason,L"FWHM seit Referenz")&&WContains(g_focusNeedReason,L"Messungen schlechter"),"3.8.8 intelligence: autofocus traffic light turns red with dynamic reason");
    ...
    total++;passed+=test(ok0&&cal0>=0&&cal0<=inc0&&ok250&&((cal250>=250)?(cal250-250):(250-cal250))<=inc250&&ok1000&&((cal1000>=1000)?(cal1000-1000):(1000-cal1000))<=inc1000&&ok5000&&((cal5000>=5000)?(cal5000-5000):(5000-cal5000))<=inc5000&&lim5000>5000,"3.8.8 simulator: backlash calibration covers 0..5000-step range");
    ...
    total++;passed+=test(!g_backlashCalActive&&WContains(controls[HI(g_status)].text,L"sichere Fokuszone reicht"),"3.8.8 simulator: insufficient safe travel fails before calibration starts");g_safeMinPosition=0;g_safeMaxPosition=g_simMaxStep;
    ...
    // 3.8.8 deep-audit regressions: focus-operation isolation and live-only trend/reference input.
    ...
    total++;passed+=test(liveBlockedByBacklash&&moveBlockedByBacklash&&learnBlockedByBacklash&&applyBlockedByBacklash,"3.8.8 audit: backlash mode blocks conflicting operations");
    ...
    total++;passed+=test(afDoesNotSetReference&&liveSetsReference,"3.8.8 audit: only completed live frames establish refocus reference");
    ...
    total++;passed+=test(pendingConsumed,"3.8.8 audit: accepted autofocus consumes pending auto-start move");
    ...
    total++;passed+=test(noBacklashResume&&noStepResume,"3.8.8 audit: forced stop never resumes live capture");

    // 3.8.8 audit regressions: stale telemetry, trustworthy rollback state and clean session statistics.
    ...
    total++;passed+=test(afFrameIgnored&&g_sessionBestFwhm==3.0&&g_sessionWorstFwhm==3.5,"3.8.8 audit: only live frames enter session FWHM range");
    ...
    total++;passed+=test(g_tempProfileCount==1&&g_tempProfileP[0]==24680,"3.8.8 audit: temperature model records confirmed final position");
    ...
    total++;passed+=test(!g_focuserConnected&&!g_hasFocuserTemperature&&g_cachedFocuserPosition==-1,"3.8.8 audit: focuser disconnect clears stale telemetry");
    ...
    total++;passed+=test(!g_afRollbackPerformed&&WContains(g_afValidationState,L"Rollback nicht möglich"),"3.8.8 audit: failed rollback is never reported as completed");
    ...
    total++;passed+=test(!g_afRollbackPerformed&&WContains(g_afValidationState,L"Abgebrochen"),"3.8.8 audit: abort clears active validation state");
    ...
    total++;passed+=test(invalidApi&&validApi,"3.8.8 audit: observatory API marks final result validity explicitly");


    // 3.8.8 numeric, persistence and native-buffer hardening.
    total++;passed+=test(QueryInt("GET /x?v=2147483647 HTTP/1.1","v=",7)==2147483647&&QueryInt("GET /x?v=-2147483648 HTTP/1.1","v=",7)==(-2147483647-1)&&QueryInt("GET /x?v=999999999999999999999999 HTTP/1.1","v=",7)==7,"3.8.8 audit: HTTP integer parsing rejects overflow");
    unsigned parsedUInt=0;int parsedInt=0;
    total++;passed+=test(ParseStrictUIntW(L"4294967295",&parsedUInt)&&parsedUInt==0xffffffffU&&!ParseStrictUIntW(L"42949672960",&parsedUInt)&&ParseStrictIntW(L"-2147483648",&parsedInt)&&parsedInt==(-2147483647-1)&&!ParseStrictIntW(L"2147483648",&parsedInt),"3.8.8 audit: profile integer parsing is range safe");
    total++;passed+=test(ParseAsciiInt("2147483647",5)==2147483647&&ParseAsciiInt("999999999999999999999",5)==5&&ParseAsciiDouble("12345678901234567890.5",12.5)>1.0e18,"3.8.8 audit: CSV number parsing avoids integer UB");
    total++;passed+=test(RoundedModelPosition(1.0e250,0,200000)==200000&&RoundedModelPosition(-1.0e250,0,200000)==0&&RoundedModelPosition(12345.49,0,200000)==12345,"3.8.8 audit: temperature model prediction is bounded before cast");
    ...
    total++;passed+=test(invalidTempIgnored&&g_tempFitValid&&g_tempSlope>49.9&&g_tempSlope<50.1,"3.8.8 audit: invalid temperature-profile points are ignored");
    ...
    total++;passed+=test(nativePrepared&&nativeRejected&&nativeBytes>=(SIZE_T)640*480*3,"3.8.8 audit: native camera buffer is preflighted safely");

    // 3.8.8 simulator profiles and calibrated mechanical backlash.
    ...
    total++;passed+=test(normalStars>=20&&fewStars<normalStars&&defocusedFwhm>normalFwhm*1.35,"3.8.8 simulator: distinct star-count and defocus profiles");
    ...
    total++;passed+=test(poorSeeingFwhm>normalFwhm*1.05&&(cloudStars<normalStars||cloudSnr<s)&&elongatedEcc>0.45,"3.8.8 simulator: poor seeing, thin-cloud and elongated profiles are distinct");
    ...
    total++;passed+=test(fabs(seeingA-seeingB)>0.08,"3.8.8 simulator: variable seeing changes focus metric");
    ...
    total++;passed+=test(calStarsA>=g_refocusMinStars&&calStarsA==calStarsB&&fabs(calA-calB)<0.0001,"3.8.8 simulator: backlash calibration stream is deterministic");
    unsigned randomSeq0=g_simProfileSelectionSequence;g_simProfileMode=SIM_RANDOM;SelectSimulatorProfile();int randomA=g_simActiveProfile;SelectSimulatorProfile();int randomB=g_simActiveProfile;total++;passed+=test(randomA>=SIM_NORMAL&&randomA<=SIM_DIFFICULT&&randomB>=SIM_NORMAL&&randomB<=SIM_DIFFICULT&&g_simProfileSelectionSequence==randomSeq0+2,"3.8.8 simulator: random profile selection is bounded and advances");
    ...
    // 3.8.8 named-profile serialization, validation, deterministic simulator seed and safe application.
    ...
    total++;passed+=test(gainField&&tamperRejected,"3.8.8 profiles: checksum rejects modified profile content");
    ProfileSnapshot invalidProfile=profileOriginal;invalidProfile.afPoints=8;total++;passed+=test(!ValidateProfileSnapshot(&invalidProfile,profileError,256),"3.8.8 profiles: invalid autofocus ranges are rejected");
    char excessiveCountProfile[65536];memcpy(excessiveCountProfile,profileJson,(SIZE_T)ALen(profileJson)+1);char* countField=strstr(excessiveCountProfile,"\"filterCount\":1");if(countField){countField+=14;countField[0]='9';countField[1]='9';countField[2]='9';countField[3]='9';countField[4]='9';countField[5]='9';}ProfileSnapshot excessiveParsed;BOOL excessiveRejected=!ParseProfileJson(excessiveCountProfile,&excessiveParsed,profileError,256);total++;passed+=test(countField&&excessiveRejected,"3.8.8 profiles: oversized collection counts are rejected before fixed arrays are accessed");
    ProfileSnapshot specialProfile=profileOriginal;CopyWBounded(specialProfile.cameraId,256,L"Camera \"gain\" detector");CopyWBounded(specialProfile.focuserId,256,L"Focuser \"filterCount\"");CopyWBounded(specialProfile.filters[0].name,32,L"Ha}band");char specialJson[65536];ProfileSnapshot specialParsed;BOOL specialRoundTrip=SerializeProfile(&specialProfile,specialJson,65536)&&ParseProfileJson(specialJson,&specialParsed,profileError,256);total++;passed+=test(specialRoundTrip&&WEquals(specialParsed.cameraId,specialProfile.cameraId)&&WEquals(specialParsed.focuserId,specialProfile.focuserId)&&WEquals(specialParsed.filters[0].name,L"Ha}band"),"3.8.8 profiles: quoted field names and braces inside strings round-trip safely");
    ProfileSnapshot invalidModel=profileOriginal;invalidModel.modelQualityMean=150.0;total++;passed+=test(!ValidateProfileSnapshot(&invalidModel,profileError,256),"3.8.8 profiles: implausible focus-model values are rejected");
    ...
    total++;passed+=test(g_simPosition==safeMotor&&g_simOpticalPosition==safeOptical&&WEquals(g_cameraId,L"Connected.Camera")&&WEquals(g_focuserId,L"Connected.Focuser")&&!g_afActive&&!g_exposing,"3.8.8 profiles: loading never moves focus or replaces connected devices");
    total++;passed+=test(!g_autoRefocusPaused&&g_autoRefocusFailureCount==0&&g_autoRefocusRetryAfter==0&&g_refocusBadFrameStreak==0&&g_profileRevision!=revisionBefore,"3.8.8 profiles: loading resets stale auto-refocus lifecycle and advances UI revision");
    ...
    total++;passed+=test(seededA==seededA2&&seededB==seededB2,"3.8.8 profiles: simulator seed reproduces random profile sequence");
    ...
    total++;passed+=test(saveCrud&&crudSlot>0&&loadCrud&&WEquals(loadedGain,L"333")&&g_simPosition==beforeLoadMotor&&g_simOpticalPosition==beforeLoadOptical,"3.8.8 profiles: named save and load persist settings without movement");
    ...
    total++;passed+=test(overwriteCrud&&reloadOverwrite&&overwrittenSlot==crudSlot&&WEquals(loadedGain,L"444"),"3.8.8 profiles: overwrite reuses the existing slot and preserves the library index");
    ...
    total++;passed+=test(exportCrud&&exportedReadable&&exportedSize>200,"3.8.8 profiles: export creates readable checked JSON");
    ...
    total++;passed+=test(importCrud&&importedSlot>0&&importedSlot!=crudSlot,"3.8.8 profiles: duplicate import gets a unique name and stays unloaded");
    ...
    total++;passed+=test(standardProtected&&activeProtected&&deleteImported,"3.8.8 profiles: protected and deletable profile rules work end-to-end");
    g_live=TRUE;BOOL busyLoadRejected=!LoadNamedProfile(L"C8 Deep Sky",crudMessage,256);total++;passed+=test(ProfileOperationBusy()&&busyLoadRejected,"3.8.8 profiles: loading is blocked during live capture");g_live=FALSE;
    ...
    total++;passed+=test(repairedStandardOk&&WEquals(repairedStandard.name,L"Standardprofil")&&repairedStandard.gain==555,"3.8.8 profiles: missing standard-profile file is repaired at startup");
    ...
    BOOL reportOk=ExportReport(L"3.8.8 Laufzeittest");int reportCsv=FakeFindFileSuffix(L"AF_20260730_120000.csv"),reportSvg=FakeFindFileSuffix(L"AF_20260730_120000.svg");
    total++;passed+=test(reportOk&&reportCsv>=0&&reportSvg>=0&&fakeFiles[reportCsv].size<32768&&fakeFiles[reportSvg].size<65536,"3.8.8 report: maximum-size CSV/SVG exports safely");
    total++;passed+=test(FakeCsvFirstPointColumns(reportCsv)==10&&FakeFileContainsBytes(reportCsv,";0.02500;")&&FakeFileContainsBytes(reportCsv,";1\r\n"),"3.8.8 report: CSV rows match ten-column header and export sigma/outlier");
    total++;passed+=test(FakeFileContainsBytes(reportSvg,"fill-opacity='0.10'")&&FakeFileContainsBytes(reportSvg,"stroke='#ff6b77' stroke-width='2'")&&FakeFileContainsBytes(reportSvg,"stroke='#66dda3'")&&FakeFileContainsBytes(reportSvg,"stroke='#54a4ff'"),"3.8.8 report: SVG contains uncertainty band, error-bar colors and red outlier cross");

    total++;passed+=test(IsSafeAscomProgIdText(L"ASCOM.Celestron.Focuser")&&IsSafeAscomProgIdText(L"ASCOM.Simulator.Camera")&&!IsSafeAscomProgIdText(L"Synthetische Kamera")&&!IsSafeAscomProgIdText(L"ASCOM/Bad.Camera"),"3.8.8 ASCOM selection: ProgID validation rejects display names and unsafe paths");
    ...
    total++;passed+=test(cameraIdAccepted&&focuserIdAccepted,"3.8.8 ASCOM selection: chooser ProgIDs transfer through the engine API");
    ...
    total++;passed+=test(liveSwapRejected,"3.8.8 ASCOM selection: connected driver cannot be replaced live");


    // 3.8.8 ToupTek runtime path validation and API persistence.
    ResetFakeProfileStorage();FakeAllocFile(L"C:\\SDK\\toupcam.dll");
    total++;passed+=test(IsSafeToupTekDllPath(L"C:\\SDK\\toupcam.dll")&&!IsSafeToupTekDllPath(L"C:\\SDK\\other.dll")&&!IsSafeToupTekDllPath(L"toupcam.dll"),"3.8.8 ToupTek: only an existing absolute toupcam.dll path is accepted");
    ...
    total++;passed+=test(toupPathAccepted,"3.8.8 ToupTek: validated SDK path is persisted with a specific API response");
    g_cameraConnected=TRUE;RunFakeHttp("GET /api/set?name=touptekDllPath&value=C%3A%5CSDK%5Ctoupcam.dll HTTP/1.1\r\nHost: 127.0.0.1\r\n\r\n");
    total++;passed+=test(strstr(fakeNetResponse,"\"ok\":false")&&strstr(fakeNetResponse,"getrennter Kamera"),"3.8.8 ToupTek: SDK path cannot change while the camera is connected");g_cameraConnected=FALSE;

    // 3.8.8 integration regressions: device selection must never open modal ASCOM UI in the hidden engine.
    ...
    total++;passed+=test(headlessCameraChooserBlocked&&headlessFocuserChooserBlocked,"3.8.8 integration: hidden engine never opens an ASCOM chooser");
    ...
    total++;passed+=test(simCameraOn&&simFocuserOn&&simFocuserOff&&simCameraOff,"3.8.8 integration: simulator camera and focuser connect and disconnect through HTTP API");
    ...
    total++;passed+=test(detailedFocuserFailure&&detailedCameraFailure,"3.8.8 integration: device worker failures preserve their concrete API reason");

    // 3.8.8 maintenance revision: atomically reserve the focuser setup window in the engine.
    ...
    total++;passed+=test(setupReserved&&setupMoveBlocked&&setupConnectBlocked&&setupDriverSwapBlocked&&setupStatusVisible&&setupReleased,"3.8.8 revision: engine setup reservation atomically blocks all focuser changes");
    ...
    total++;passed+=test(faultedSetupRejected&&blockedWorkerRejected&&wantedSetupRejected,"3.8.8 revision: setup requires a clean fully disconnected and unwanted worker state");
    ...
    total++;passed+=test(directWorkerBlocked,"3.8.8 revision: direct ASCOM focuser worker creation is blocked during setup reservation");
