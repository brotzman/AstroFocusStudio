...
BOOL WINAPI WriteFile(HANDLE,LPCVOID,DWORD,DWORD*,LPVOID){return FALSE;}
...
int main(){
    ...
    total++;pass+=check(WEqual(g_state.version,L"3.8.8"),"Frontend: exakte Engine-Version geparst");
    ...
    char toupErrorResponse[512];const char* toupErrorBody="{\"ok\":false,\"message\":\"Die 64-Bit-toupcam.dll wurde nicht gefunden oder ist inkompatibel.\"}";sprintf(toupErrorResponse,"HTTP/1.1 200 OK\r\nContent-Length: %d\r\nConnection: close\r\n\r\n%s",(int)strlen(toupErrorBody),toupErrorBody);fakeHttp=toupErrorResponse;fakeHttpSize=(int)strlen(toupErrorResponse);fakeHttpPos=0;BOOL toupRejected=!ApiPath("/api/action?name=camera-connect");total++;pass+=check(toupRejected&&!g_lastApiTransportFailure&&WTextContains(g_lastApiMessage,L"toupcam.dll"),"3.8.8 frontend: genaue ToupTek-Engineursache bleibt erhalten");
    hFocBackend=FH(1);fakeComboSel[1]=0;g_ascomChooserProcess=0;fakeConnectCount=0;fakeHttp=responseTrue;fakeHttpSize=(int)strlen(responseTrue);fakeHttpPos=0;FocuserConnect();total++;pass+=check(fakeConnectCount>=1&&g_ascomChooserProcess==0,"3.8.8 frontend: Simulator-Fokussierer verbindet ohne ASCOM-Chooser");
    ...
    total++;pass+=check(TabIndexFromId(ID_TAB_LIVE)==0&&TabIndexFromId(ID_TAB_AUTO)==1&&TabIndexFromId(ID_TAB_BAHT)==2&&TabIndexFromId(ID_TAB_LOG)==3&&TabIndexFromId(ID_TAB_AUTOMATION)==4&&TabIndexFromId(ID_TAB_ANALYSIS)==5&&TabIndexFromId(ID_TAB_TESTS)==6&&TabIndexFromId(ID_TAB_FIELD)==-1,"Frontend: sichtbare Reiter ohne doppeltes Bildfeld korrekt gemappt");
    ...
    total++;pass+=check(LooksLikeAscomProgId(L"ASCOM.Celestron.Focuser")&&LooksLikeAscomProgId(L"ASCOM.Simulator.Camera"),"3.8.8 frontend: gültige ASCOM-ProgIDs werden erkannt");
    total++;pass+=check(!LooksLikeAscomProgId(L"Synthetische Kamera")&&!LooksLikeAscomProgId(L"ToupTek Camera")&&!LooksLikeAscomProgId(L""),"3.8.8 frontend: Geräteanzeigenamen gelten nicht als ASCOM-Auswahl");
    total++;pass+=check(!LooksLikeAscomProgId(L"ASCOM/Bad.Camera")&&!LooksLikeAscomProgId(L"ASCOM.Bad Camera"),"3.8.8 frontend: unsichere ASCOM-Auswahlwerte werden verworfen");
    wchar_t chooserOut[64];
    const wchar_t validChooserId[]=L"ASCOM.Celestron.Focuser";
    total++;pass+=check(CopyValidatedAscomProgId(validChooserId,lstrlenW(validChooserId),chooserOut,64)&&LooksLikeAscomProgId(chooserOut),"3.8.8 frontend: vollständige Chooser-ProgID wird unverändert übernommen");
    const wchar_t embeddedNul[]={L'A',L'S',L'C',L'O',L'M',L'.',L'X',0,L'Y'};
    total++;pass+=check(!CopyValidatedAscomProgId(embeddedNul,9,chooserOut,64)&&chooserOut[0]==0,"3.8.8 frontend: eingebettetes NUL mit Folgedaten wird verworfen");
    const wchar_t controlId[]={L'A',L'S',L'C',L'O',L'M',L'.',L'X',L'\n',L'Y'};
    total++;pass+=check(!CopyValidatedAscomProgId(controlId,9,chooserOut,64),"3.8.8 frontend: Steuerzeichen im Chooser-Ergebnis werden verworfen");
    total++;pass+=check(!CopyValidatedAscomProgId(validChooserId,lstrlenW(validChooserId),chooserOut,8),"3.8.8 frontend: zu kleine Zielpuffer führen nicht zu Kürzung");
    total++;pass+=check(ClassifyFocuserSetupExitCode(0)==0&&ClassifyFocuserSetupExitCode(10)==1&&ClassifyFocuserSetupExitCode(6)==2&&ClassifyFocuserSetupExitCode(0xffffffffUL)==2,"3.8.8 frontend: Setup-Exitcodes werden als Erfolg, Abbruch oder Fehler klassifiziert");
    // 3.8.8 chooser integration: opening a modal helper freezes all backend-changing controls.
    ...
    total++;pass+=check(chooserControlsLocked&&chooserControlsRestored,"3.8.8 integration: chooser locks and restores all backend-changing controls");
    ...
    total++;pass+=check(validChooserState&&changedCameraRejected&&changedFocuserRejected,"3.8.8 integration: chooser result is discarded after a backend change");
    ...
    total++;pass+=check(g_uvcProcess==0&&fakeCloseCount==closesBefore+1&&fakeText[13][0]!=0,"3.8.8 frontend: beendeter UVC-Prozess trennt Status sauber");
    ...
    total++;pass+=check(fakeText[0][0]==L'1'&&fakeText[1][0]==L'3'&&fakeText[15][0]==L'2'&&fakeText[14][0]==L'P'&&fakeText[28][0]==L'6',"3.8.8 frontend: profile revision synchronizes loaded settings into existing controls");
    SetWindowTextW(hGain,L"999");AppState sameRevision=g_state;UpdateUiFromState(&sameRevision);total++;pass+=check(fakeText[1][0]==L'9',"3.8.8 frontend: ordinary polling preserves unsaved control edits without a new profile revision");
    ...
    total++;pass+=check(narrowLampH>148&&narrowLampH<=260&&wideLampH>=148&&wideLampH<=narrowLampH,"3.8.8 frontend: lange Automatikbegründung vergrößert Karte ohne Clipping");
    memset(&g_state,0,sizeof(g_state));g_state.focuserMoving=TRUE;
    total++;pass+=check(FocuserOperationBusy(),"3.8.8 frontend: laufende Motorbewegung sperrt Setup und Backendwechsel");
    g_state.focuserMoving=FALSE;g_state.stepAssistantActive=TRUE;
    total++;pass+=check(FocuserOperationBusy(),"3.8.8 frontend: Schritt-Assistent sperrt Treiberkalibrierung");
    g_state.stepAssistantActive=FALSE;g_state.backlashCalibrationActive=TRUE;
    total++;pass+=check(FocuserOperationBusy(),"3.8.8 frontend: Backlash-Kalibrierung sperrt Treiberkalibrierung");
    ParseStatus("{\"version\":\"3.8.8\",\"focuserSetupActive\":true}");
    total++;pass+=check(g_state.focuserSetupActive&&FocuserOperationBusy(),"3.8.8 revision: engine setup reservation is parsed and locks the UI");
    g_state.focuserSetupActive=FALSE;g_focuserSetupEngineLock=TRUE;
    total++;pass+=check(FocuserOperationBusy(),"3.8.8 revision: local setup launch window remains locked before next poll");g_focuserSetupEngineLock=FALSE;
