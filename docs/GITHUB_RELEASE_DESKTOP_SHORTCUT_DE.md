...
Der Workflow kann normal zum reinen Bauen und Testen gestartet werden. Für eine Veröffentlichung wird unter **Run workflow** `publish_release` aktiviert und `release_tag` auf `v3.9.0` gesetzt. Alle Build-, Sanitizer-, Bundle- und MSI-Tests müssen erfolgreich sein. Danach prüft der Release-Job die SHA-256-Datei, erzeugt einen fehlenden Tag am getesteten Commit und erstellt oder aktualisiert das GitHub Release.

Ein abweichender Tag wird abgelehnt, damit ein 3.9.0-Paket nicht versehentlich unter einer anderen Versionsnummer veröffentlicht wird. Ein normal gepushter Tag `v3.9.0` löst denselben geprüften Releasepfad aus.
