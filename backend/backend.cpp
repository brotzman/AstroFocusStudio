// AstroFocus Studio current version Engine
...
static HMODULE g_ws2=0; static SOCKET g_httpListen=(SOCKET)(~0ULL); static int g_httpPort=AFS_LOCAL_HTTP_PORT; static BOOL g_shutdownRequested=FALSE;
...
static BOOL SaveDiagnostics(){
    ...
    int n=sprintf(b,
        "AstroFocus Studio " AFS_VERSION_TEXT_A " Engine\r\n"
        "Kamera=%s\r\nFokussierer=%s\r\nToupTekSDK=%s\r\nSimulator=%d\r\nExternal=%d\r\nNative=%d\r\n"
...
static const char g_webUi[] = R"CAFHTML(<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AstroFocus Studio )CAFHTML" AFS_VERSION_TEXT_A R"CAFHTML( Engine</title><style>
...
static BOOL HeaderValueIsLocal(const char* request,const char* header){
    ...
    p+=ALen(header);
    while(*p==' '||*p=='\t')p++;
    return LocalUrlPrefix(p,AFS_LOCAL_HTTP_BASE_A)||
           LocalUrlPrefix(p,"http://localhost:" AFS_LOCAL_HTTP_PORT_TEXT_A);
}
...
static void OpenWebUi(){if(!g_shell32)g_shell32=LoadLibraryW(L"shell32.dll");if(!g_shell32)return;if(!pShellExecuteW)pShellExecuteW=(PFN_ShellExecuteW)GetProcAddress(g_shell32,"ShellExecuteW");if(pShellExecuteW)pShellExecuteW(0,L"open",L"msedge.exe",L"--app=" AFS_LOCAL_HTTP_BASE_W L"/ --start-maximized --no-first-run --disable-extensions",0,1);}
...
static void SendObservatoryStatus(SOCKET c){
    ...
    char body[8192];int n=sprintf(body,
        "{\"apiVersion\":1,\"app\":\"AstroFocus Studio\",\"version\":\"" AFS_VERSION_TEXT_A "\","
...
static void SendFocusModelStatus(SOCKET c){
    char body[8192];int n=sprintf(body,"{\"apiVersion\":1,\"version\":\"" AFS_VERSION_TEXT_A "\",\"valid\":%s,\"slopeStepsPerC\":%.6f,\"intercept\":%.3f,\"count\":%d,\"temperatureC\":[",g_tempFitValid?"true":"false",JsonNumber(g_tempSlope),JsonNumber(g_tempIntercept),g_tempProfileCount);
...
static void CommitBacklashCalibrationEstimate(int estimated){
    estimated=ClampI(estimated,0,1000000);g_backlashCalEstimated=estimated;g_afBacklash=estimated;
    wchar_t value[32];wsprintfW(value,L"%d",estimated);SetWindowTextW(g_afBacklashEdit,value);
    g_profileBacklash=estimated;SaveSettings();SaveFocusProfileModel();
}
static void HandleBacklashCalibrationFrame(){
...
static BOOL ExportReport(LPCWSTR result){
    ...
    m+=sprintf(svg+m,"<svg xmlns='http://www.w3.org/2000/svg' width='1000' height='650' viewBox='0 0 1000 650'><rect width='1000' height='650' fill='#171c24'/><text x='40' y='42' fill='#f5f7fa' font-family='Segoe UI' font-size='25'>AstroFocus Studio " AFS_VERSION_TEXT_A " Engine</text><text x='40' y='72' fill='#aeb8c6' font-family='Segoe UI' font-size='16'>%s - %s</text><text x='70' y='101' fill='#91a6bf' font-family='Segoe UI' font-size='14'>HFR grün · FWHM blau · ±1σ-Fehlerbalken · rotes × Ausreißer · □ Kontrollmedian</text><rect x='70' y='115' width='860' height='400' fill='#11161d' stroke='#465060'/>",profile,resultA);
...
static void PaintMain(HDC dc){
    ...
    SetBkMode(dc,TRANSPARENT);HFONT of=(HFONT)SelectObject(dc,g_titleFont);SetTextColor(dc,RGB(244,247,252));TextOutW(dc,22,17,AFS_ENGINE_TITLE_W,WLen(AFS_ENGINE_TITLE_W));SelectObject(dc,of);
...
static void LoadSettings(){
    ...
    // The central loopback port is the mandatory local IPC channel used by the visible frontend.
    g_httpEnabled=TRUE;
