// AstroFocus Studio 3.8.8 Engine
...
static const char g_webUi[] = R"CAFHTML(<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AstroFocus Studio 3.8.8 Engine</title><style>
...
static void SendObservatoryStatus(SOCKET c){
    ...
    char body[8192];int n=sprintf(body,
        "{\"apiVersion\":1,\"app\":\"AstroFocus Studio\",\"version\":\"3.8.8\","
...
static void SendFocusModelStatus(SOCKET c){
    char body[8192];int n=sprintf(body,"{\"apiVersion\":1,\"version\":\"3.8.8\",\"valid\":%s,\"slopeStepsPerC\":%.6f,\"intercept\":%.3f,\"count\":%d,\"temperatureC\":[",g_tempFitValid?"true":"false",JsonNumber(g_tempSlope),JsonNumber(g_tempIntercept),g_tempProfileCount);
...
static void PollHttpServer(){
    ...
    n+=sprintf(body+n,"\"native\":%s,\"reconnect\":%s,\"simulator\":%s,\"external\":%s,\"demo\":%s,\"stretch\":%s,\"stars\":%s,\"values\":%s,\"saturation\":%s,\"zones\":%s,\"bahtinov\":%s,\"preferredOut\":%s,\"autoRefocus\":%s,\"tempComp\":%s,\"autoStartPositionEnabled\":%s}",IsChecked(g_nativeCheck)?"true":"false",IsChecked(g_reconnectCheck)?"true":"false",g_simulatorMode?"true":"false",g_externalCameraMode?"true":"false",g_demo?"true":"false",IsChecked(g_autoStretch)?"true":"false",IsChecked(g_overlayStars)?"true":"false",IsChecked(g_overlayLabels)?"true":"false",IsChecked(g_overlaySat)?"true":"false",IsChecked(g_zoneCheck)?"true":"false",IsChecked(g_bahtinovCheck)?"true":"false",IsChecked(g_preferredDirCheck)?"true":"false",g_autoRefocusEnabled?"true":"false",g_tempCompEnabled?"true":"false",g_autoStartPositionEnabled?"true":"false");if(n<=0||n>=statusCapacity){const char* e="{\"ok\":false,\"message\":\"Statusantwort zu groß\"}";SendHttp(c,"application/json",e,ALen(e));}else SendHttp(c,"application/json",body,n);Free(body);p_closesocket(c);
}
...
static BOOL ExportReport(LPCWSTR result){
    ...
    m+=sprintf(svg+m,"<svg xmlns='http://www.w3.org/2000/svg' width='1000' height='650' viewBox='0 0 1000 650'><rect width='1000' height='650' fill='#171c24'/><text x='40' y='42' fill='#f5f7fa' font-family='Segoe UI' font-size='25'>AstroFocus Studio 3.8.8 Engine</text><text x='40' y='72' fill='#aeb8c6' font-family='Segoe UI' font-size='16'>%s - %s</text><text x='70' y='101' fill='#91a6bf' font-family='Segoe UI' font-size='14'>HFR grün · FWHM blau · ±1σ-Fehlerbalken · rotes × Ausreißer · □ Kontrollmedian</text><rect x='70' y='115' width='860' height='400' fill='#11161d' stroke='#465060'/>",profile,resultA);
...
static void ProcessCompletedFrame(int purpose){
    ...
    if(purpose==1){if(g_live)StartLiveExposureOrStop(L"Live-Fokus konnte die nächste Belichtung nicht starten");}else if(purpose==4)HandleStepAssistantFrame();else if(purpose==5)HandleBacklashCalibrationFrame();else if(purpose==2||purpose==3)HandleAutofocusFrame(purpose);
}
...
static BOOL TriggerAutomaticRefocus(LPCWSTR reason){
    ULONGLONG now=GetTickCount64();if(g_autoRefocusPaused||now<g_autoRefocusRetryAfter||g_afActive||g_stepAssistActive||g_backlashCalActive)return FALSE;
...
static void PaintMain(HDC dc){
    ...
    SetBkMode(dc,TRANSPARENT);HFONT of=(HFONT)SelectObject(dc,g_titleFont);SetTextColor(dc,RGB(244,247,252));TextOutW(dc,22,17,L"AstroFocus Studio 3.8.8 Engine",27);SelectObject(dc,of);
